Shader "Hidden/EpilepsySafeTransitionEffect"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed4 lines = max(normalValue, depthValue);

                fixed4 second = (1 - lines) * _SecondaryColor;
                lines *= _Color;

                lines += second;

                _Fade = clamp(_Fade, 0,1);
                fixed4 col = lerp(tex2D(_MainTex, i.uv), lines, _Fade);
